...
def build_annual_stream(exact_age, monthly_annuity, segment_rates, mortality_table):
    ...
    # === START FIX: Use the user's reverse-engineered offset ===
    offset = -0.5835038 
    # === END FIX ===

    total_years = 64 # Max 64 years, per user's reverse-engineering
    annual_annuity = monthly_annuity * 12
    
    df = pd.DataFrame(index=range(1, total_years + 1))
    df.index.name = "Year"
    
    # === START FIX: Remove np.floor to use fractional age ===
    # This was the bug causing the scaling to fail.
    # OLD: df["Age"] = np.floor(exact_age + df.index)
    df["Age"] = exact_age + df.index
    # === END FIX ===

    # Interpolate cumulative survival
    df["Survival"] = np.interp(
        df["Age"], 
        mortality_table.index, # This is the Age
        mortality_table.values # This is the cum_survival
    )

    # Assign segment rates
    df["Segment Rate (Annual)"] = segment_rates["seg1"] # Default seg1
...
def simulate_retirement_window(birth_date, start_date, months, salary, base_years, rate_tables, mortality_table, partial_percent, assumed_growth):
    """
    ...
    for i in range(months):
        ...
        # === START FIX: Correctly determine the plan year for rates ===
        # The plan year switches on Feb 1, not Jan 1.
        plan_year = retire_date.year
        if retire_date.month == 1: # If it's Jan (e.g., 2030-01-01)
            plan_year = retire_date.year - 1 # Use the previous year's rates (2029)
        
        year = plan_year
        # === END FIX ===
        
        if year not in rate_tables:
            year = max(rate_tables.keys()) # Use last available year if no future data
...
def simulate_segment_rate_sensitivity(exact_age, monthly_annuity, mortality_table, base_rates):
    ...
    # Base case
    stream = build_annual_stream(exact_age, monthly_annuity, base_rates, mortality_table)
    ...
    for i in [-0.01, -0.005, 0.005, 0.01]: # +/- 1% and 0.5%
        for seg in ["seg1", "seg2", "seg3"]:
            ...
            stream = build_annual_stream(exact_age, monthly_annuity, rates, mortality_table)
...
def run_feb_1_projection(birth_date, salary, base_years, partial_percent, rate_tables, assumed_growth, feb_1_date, mortality_table):
    ...
    if feb_1_date.month == 1: # If it's Jan (e.g., 2030-01-01)
        plan_year = feb_1_date.year - 1 # Use the previous year's rates (2029)
    ...
    # Get adjusted inputs
    # We pass the *retire_date* (feb_1_date) to get_adjusted_inputs
    # === START FIX: Correct variable name ===
    # OLD: adjusted_salary, adjusted_years = get_adjusted_inputs(
    #         salary, base_years, retire_date, last_emp_date, assumed_growth
    #     )
    adjusted_salary, adjusted_years = get_adjusted_inputs(
    ...
    return {
        ...
        # "Partial Lump Sum": partial_llump,
...
def run_simulation(birth_date, retire_date, salary, base_years, partial_percent, rate_tables, assumed_growth, mortality_table_csv):
    ...
    # === Initial Calculation (for Summary Panel) ===
    
    # 1. Load data
    mortality_table = load_irs_unisex_mortality(mortality_table_csv)
    if mortality_table is None: # Stop if table failed to load
        return None 
        
    initial_last_emp_date = retire_date - relativedelta(days=1)
    
    # 2. Get rates for the *initial* date
    # === START FIX: Correctly determine the plan year for rates ===
    plan_year = retire_date.year
    if retire_date.month == 1: # If it's Jan (e.g., 2030-01-01)
        plan_year = retire_date.year - 1 # Use the previous year's rates (2029)
    ...
    # 3. Get adjusted inputs for the *initial* date
    adjusted_salary, adjusted_years = get_adjusted_inputs(
    ...
    # 4. Calculate initial annuity and lump sum
    monthly_annuity = calculate_monthly_annuity(adjusted_salary, adjusted_years)
    ...
    # === Feb 1st Projection (for Summary Panel) ===
    
    # Find the next Feb 1st
    ...
    # === START FIX: Correct typo ===
    # OLD: feb_1_summary = run_feb_1__projection(

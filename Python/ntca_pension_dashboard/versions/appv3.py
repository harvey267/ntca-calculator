...
from pension_model import build_ntca_annuity_table
...
with st.sidebar.expander("🧮 Discount Offset Logic"):
    offset_mode = st.selectbox("Offset Mode", ["Dynamic (Day-Level)", "Fixed (NTCA-style)"])
...
try:
    results = run_simulation(
        ...
        mortality_table_csv,
        offset_mode  # ✅ Positional argument
    )
...
def compare_with_ntca(sim_df, ntca_file):
    import pandas as pd
    import streamlit as st

    try:
        ntca_df = pd.read_excel(ntca_file)
        ntca_df.columns = ntca_df.columns.str.strip()
        sim_df.columns = sim_df.columns.str.strip()

        # Ensure both datasets have comparable Retirement Date formats
        sim_df["Retirement Date"] = pd.to_datetime(sim_df["Retirement Date"]).dt.date
        ntca_df["Retirement Date"] = pd.to_datetime(ntca_df["Retirement Date"]).dt.date
        ...
        for col in ["Retirement Date", "Exact Age"]:
            if col in merged.columns:
                columns_to_show.append(col)

        if "NTCA Lump Sum" in merged.columns and "Full Lump Sum" in merged.columns:
            merged["Δ Lump Sum"] = merged["Full Lump Sum"] - merged["NTCA Lump Sum"]
            merged["% Δ Lump Sum"] = merged["Δ Lump Sum"] / merged["NTCA Lump Sum"] * 100
            columns_to_show += ["NTCA Lump Sum", "Full Lump Sum", "Δ Lump Sum", "% Δ Lump Sum"]

        if "NTCA Annuity" in merged.columns and "Monthly Annuity" in merged.columns:
            merged["Δ Monthly Annuity"] = merged["Monthly Annuity"] - merged["NTCA Annuity"]
            merged["% Δ Monthly Annuity"] = merged["Δ Monthly Annuity"] / merged["NTCA Annuity"] * 100
            columns_to_show += ["NTCA Annuity", "Monthly Annuity", "Δ Monthly Annuity", "% Δ Monthly Annuity"]

        st.header("📊 NTCA Comparison Panel")
        st.dataframe(merged[columns_to_show].style.format({
            "NTCA Lump Sum": "${:,.2f}",
            "Full Lump Sum": "${:,.2f}",
            "Δ Lump Sum": "${:,.2f}",
            "% Δ Lump Sum": "{:+.2f}%",
            "NTCA Annuity": "${:,.2f}",
            "Monthly Annuity": "${:,.2f}",
            "Δ Monthly Annuity": "${:,.2f}",
            "% Δ Monthly Annuity": "{:+.2f}%"
        }))

    except Exception as e:
        st.error("⚠️ Failed to compare with NTCA spreadsheet.")
        st.exception(e)
...
if results:
    ...
    with st.expander("📆 NTCA-Style Monthly Annuity Table"):
        ntca_annuity_df = build_ntca_annuity_table(
            high5_salary=126339.20,
            base_years=26.1288,
            months=24,
            start_date=date(2029, 6, 1)
        )
        st.dataframe(ntca_annuity_df.style.format({
            "Monthly Annuity": "${:,.2f}",
            "Yearly Annuity": "${:,.2f}",
            "Service Years": "{:,.4f}"
        }))

        csv_annuity = convert_df(ntca_annuity_df)
        st.download_button(
            label="Download NTCA Annuity Table",
            data=csv_annuity,
            file_name="ntca_annuity_table.csv",
            mime='text/csv',
        )

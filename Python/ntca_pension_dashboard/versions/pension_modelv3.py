...
def get_adjusted_inputs(base_salary, base_years, calc_date, last_emp_date, assumed_growth):
    ...
    Calibrated to match NTCA's monthly annuity increments and buy-out logic.
    """
...
def simulate_retirement_window(
    ...
    rate_tables, mortality_table, partial_percent, assumed_growth, offset_mode
):
    """
...
def simulate_segment_rate_sensitivity(exact_age, monthly_annuity, mortality_table, base_rates, discount_offset):
    """
...
def run_feb_1_projection(
    ...
    rate_tables, assumed_growth, feb_1_date, mortality_table, offset_mode
):
    """
...
def build_ntca_annuity_table(high5_salary, base_years, benefit_rate=0.019, months=24, start_date=date(2029, 6, 1)):
    """
    Builds a table of monthly annuity values using NTCA-style logic:
    - High-5 salary is fixed
    - Service years increment monthly
    ...
    for i in range(months):
        retire_date = start_date + relativedelta(months=i)
        fractional_years = i / 12.0
        service_years = base_years + fractional_years

        monthly_annuity = (high5_salary * service_years * benefit_rate) / 12.0
        yearly_annuity = monthly_annuity * 12

        rows.append({
            "Retirement Date": retire_date,
            "Service Years": round(service_years, 4),
            "Monthly Annuity": round(monthly_annuity, 2),
            "Yearly Annuity": round(yearly_annuity, 2)
        })
...
def run_simulation(
    ...
    rate_tables, assumed_growth, mortality_table_csv, offset_mode  # ← Add this
):
    """
    ...
    df_retire_sim = simulate_retirement_window(
        ...
        offset_mode=offset_mode  # ✅ Add this
    )
